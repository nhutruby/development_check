class AccountTypesController < ApplicationController
  ...
  def index
    ...
    #@products.sort! {|a, b| a.price_in_cents <=> b.price_in_cents} # Sort by price ascendant
  ...
  def create
    ...
    respond_to do |format|
      if @account_type.save
        format.html { redirect_to(account_types_path, :notice => 'Account type was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @account_type.update_attributes(params[:account_type])
        format.html { redirect_to(account_types_path, :notice => 'Account type was successfully updated.') }
        format.xml  { head :ok }
      else
